...
class ClienteServiceTest {
    ...
    void criar_deveSalvarQuandoEmailNaoExiste() {
        ...
        c.setNome("Maria");
        c.setEmail("m@ex.com");
        when(repo.existsByEmail("m@ex.com")).thenReturn(false);
        when(repo.save(any())).thenAnswer(i -> {
            Cliente a = i.getArgument(0);
            a.setId(5L);
            return a;
        });
        ...
        assertThat(salvo.getId()).isEqualTo(5L);
        verify(repo).save(any(Cliente.class));
    }

    @Test
    void buscar_deveLancarNotFoundQuandoAusente() {
        when(repo.findById(99L)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> service.buscar(99L)).isInstanceOf(com.teste.spring.teste.exception.NotFoundException.class);
    }

    @Test
    void excluir_deveChamarDeleteQuandoExiste() {
        Cliente c = new Cliente();
        c.setId(7L);
