...
public class ClienteControllerTest {
    ...
    void criar_deveRetornarCreated() throws Exception {
        ...
        dto.setNome("Teste");
        dto.setEmail("t@ex.com");

        Cliente saved = new Cliente();
        saved.setId(12L);
        saved.setNome("Teste");
        saved.setEmail("t@ex.com");

        when(service.criar(any())).thenReturn(saved);

        mvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post("/api/clientes")
                        .contentType(org.springframework.http.MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(dto)))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.status().isCreated())
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$.id").value(12))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$.nome").value("Teste"));
    }
}
